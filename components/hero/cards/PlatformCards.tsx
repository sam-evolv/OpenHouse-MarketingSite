 ...
 *
 * The Property Assistant is the focal surface, because it is where the
 * record resolves into an answer. The dashboard sits above it as the
 * control plane; Care and the energy layer sit below as extensions.
 *
 * Geometry is deliberate and shared. Cards sit in two columns with a
 * clear channel between them; the record spine runs down that channel
 * and each view taps into it with a short stub, routed orthogonally like
 * a services riser so every run sits in negative space instead of
 * disappearing under a card. Stub lengths depend only on card width and
 * centre, both exact here. Gold carries the verified record and the
...
export function PlatformFloatingCards() {
  return (
    <>
      ...
      >
        ...
        {STUBS.map((s) => (
          <span
            ...
            style={{ width: s.width, transform: `translate(${s.x}px, ${s.y}px)` }}
          >
            <span
      ...
      {/* Card 2: Property Assistant — the focal surface, where the record
          resolves into an answer */}
      <FloatingCard
        depth={1}
        elevation={4}
      ...
      >
        <div className="w-52 card-surface rounded-2xl overflow-hidden border-emerald-500/25">
          <div className="p-4">
