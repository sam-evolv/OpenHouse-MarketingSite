        ...
        <div className="flex items-center justify-between h-32 sm:h-36">
          ...
          <div className="hidden md:flex items-center space-x-3">
            <div className="relative" onMouseEnter={() => setShowLoginTooltip(true)} onMouseLeave={() => setShowLoginTooltip(false)}>
              <a href={appRoutes.login} className="group relative flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-full overflow-hidden transition-all duration-300 border border-white/20 hover:border-gold/60 bg-white/[0.03] hover:bg-white/[0.08]">
              ...
              <div className={cn("absolute top-full left-1/2 -translate-x-1/2 mt-2 transition-all duration-200", showLoginTooltip ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-1 pointer-events-none")}>
                <div className="relative bg-slate border border-white/10 rounded-lg p-3 shadow-xl min-w-[200px]">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-gold/20 flex items-center justify-center"><ExternalLink className="w-4 h-4 text-gold" /></div>
                    <div><p className="text-sm font-medium text-porcelain">Developer Dashboard</p><p className="text-xs text-hint">Access your portal</p></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden text-porcelain hover:text-gold" aria-label={isMobileMenuOpen ? "Close navigation menu" : "Open navigation menu"} aria-expanded={isMobileMenuOpen} aria-controls="mobile-menu">
        ...
        {isMobileMenuOpen && (
          <div id="mobile-menu" className="md:hidden py-4 border-t border-hint/10" role="menu">
            <div className="flex flex-col space-y-4">
              ...
              <a href="/contact" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium text-gold hover:text-gold/80 px-1">Book a Demo</a>
            </div>
          </div>
        )}
      </div>
