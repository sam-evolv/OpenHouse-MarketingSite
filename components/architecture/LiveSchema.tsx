    ...
    >
      <motion.div
        className={`relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl border-2 flex items-center justify-center cursor-pointer transition-all duration-300 ${
          isHovered 
            ? "border-cyan-400 bg-cyan-500/20 shadow-[0_0_30px_rgba(6,182,212,0.5)]" 
      ...
      >
        ...
        />
        <Icon className={`w-8 h-8 sm:w-10 sm:h-10 transition-colors ${isHovered ? "text-cyan-300" : "text-cyan-400"}`} />
      ...
      <div className="mt-4 text-center">
        ...
        <p className="text-xs text-cyan-400/70">{node.sublabel}</p>
      </div>
      ...
      <AnimatePresence>
        {isHovered && node.tooltip && (
          ...
          >
            <div className="relative bg-slate-800 border border-cyan-500/30 rounded-lg px-4 py-2 shadow-xl whitespace-nowrap">
              <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-3 h-3 bg-slate-800 border-l border-t border-cyan-500/30 rotate-45" />
              <p className="text-xs text-cyan-300 font-mono">{node.tooltip}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function DataPacket({ direction }: { direction: "forward" | "backward" }) {
  return (
    <motion.div
      className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.8)]"
    ...
    <div className="relative">
      ...
      <div className="md:hidden flex flex-col items-center gap-4 mt-4">
        {[0, 1, 2].map(i => (
          <motion.div
            key={i}
            className="w-0.5 h-8 bg-gradient-to-b from-cyan-500/60 to-cyan-500/20"
...
export function LiveSchema() {
  return (
    <section className="py-28 relative overflow-hidden" style={{ backgroundColor: "#0c1929" }}>
      <BlueprintGrid />
      
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#0c1929] to-transparent opacity-80" />
        ...
        <div className="mt-16 relative">
          ...
          >
            {[
              ...
              { label: "Accuracy", value: "99.2%", color: "text-cyan-400" },
              { label: "Uptime SLA", value: "99.9%", color: "text-gold" },
            ].map((stat) => (
